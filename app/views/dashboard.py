...
def get_metrics():
    ...
    queries = max(len(st.session_state.metrics["latency_history"]), 1)  # éviter division par zéro
    return {
...
def dashboard_page():
    ...
    if len(st.session_state.metrics["latency_history"]) > 0:
        ...
        with col1:
            fig1 = px.line(x=list(range(1, len(st.session_state.metrics["latency_history"]) + 1)), 
        ...
        with col2:
            fig2 = px.line(x=list(range(1, len(st.session_state.metrics["cost_history"])+ 1)), 
        ...
        with col3:
            ...
            values = [sum(st.session_state.metrics["cost_history"]), sum(st.session_state.metrics["carbon_history"])]
            fig3 = px.pie(names=labels, values=values, title="Répartition des dépenses et impact écologique",
